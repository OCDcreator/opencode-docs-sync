...
| ---------------------------------------- | ------- | ----------------------------------------------------------------- |
|  | `-c`    | متابعة الجلسة الأخيرة                                             |
|   | `-s`    | معرّف الجلسة للمتابعة                                             |
|      |         | تفريع الجلسة عند المتابعة (يستخدم مع `--continue` أو `--session`) |
|    |         | الموجّه المراد استخدامه                                           |
|     | `-m`    | النموذج المراد استخدامه بصيغة provider/model                      |
|     |         | الوكيل المراد استخدامه                                            |
|      |         | المنفذ الذي يتم الاستماع عليه                                     |
|  |         | اسم المضيف الذي يتم الاستماع عليه                                 |
...
| ---------------------------------------- | ------- | --------------------------------------------------------------------------------- |
|       |         | دليل العمل الذي ستبدأ منه واجهة TUI                                               |
|  | `-c`    | متابعة آخر جلسة                                                                   |
|   | `-s`    | معرّف الجلسة للمتابعة                                                             |
|      |         | تفريع الجلسة عند المتابعة (استخدمه مع `--continue` أو `--session`)                |
|  | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
|  | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `opencode`) |
...
| الراية                                | الوصف                                |
| ------------------------------------- | ------------------------------------ |
|  | حدث GitHub مُحاكى لتشغيل الوكيل عليه |
...
| الراية                                  | الوصف                                                         |
| --------------------------------------- | ------------------------------------------------------------- |
|  | تحديث ذاكرة التخزين المؤقت للنماذج من models.dev              |
|  | استخدام مخرجات أكثر تفصيلا للنماذج (تشمل بيانات مثل التكاليف) |
...
| الراية                                   | المختصر | الوصف                                                                             |
| ---------------------------------------- | ------- | --------------------------------------------------------------------------------- |
|   |         | الأمر المراد تشغيله؛ استخدم الرسالة كوسائط                                        |
|  | `-c`    | متابعة الجلسة الأخيرة                                                             |
|   | `-s`    | معرّف الجلسة للمتابعة                                                             |
|      |         | تفريع الجلسة عند المتابعة (يستخدم مع `--continue` أو `--session`)                 |
|     |         | مشاركة الجلسة                                                                     |
|     | `-m`    | النموذج المراد استخدامه بصيغة provider/model                                      |
|     |         | الوكيل المراد استخدامه                                                            |
|      | `-f`    | ملف/ملفات لإرفاقها بالرسالة                                                       |
|    |         | التنسيق: default (منسق) أو json (أحداث JSON خام)                                  |
|     |         | عنوان للجلسة (يستخدم موجهًا مقتطعًا إن لم تُحدَّد قيمة)                           |
|    |         | الإرفاق بخادم opencode قيد التشغيل (مثل http://localhost:4096)                    |
|  | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
|  | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `opencode`) |
|       |         | دليل التشغيل، أو المسار على الخادم البعيد عند الإرفاق                             |
|   |         | متغير النموذج (جهد الاستدلال الخاص بالمزود)                                       |
|  |         | عرض كتل التفكير                                                                   |
|      |         | منفذ الخادم المحلي (الافتراضي منفذ عشوائي)                                        |
...
|  | اسم المضيف الذي يتم الاستماع عليه         |
|      | تفعيل اكتشاف mDNS                         |
|      | أصول/منشأات إضافية للمتصفح للسماح بـ CORS |
...
| الراية                                    | المختصر | الوصف                                 |
| ----------------------------------------- | ------- | ------------------------------------- |
|  | `-n`    | حصر النتائج في أحدث N جلسات           |
|     |         | تنسيق المخرجات: table أو json (table) |
...
| الراية                                  | الوصف                                                                     |
| --------------------------------------- | ------------------------------------------------------------------------- |
|     | عرض الإحصاءات لآخر N يومًا (الافتراضي: كل الوقت)                          |
|    | عدد الأدوات المطلوب عرضها (الافتراضي: الكل)                               |
|   | عرض تفصيل استخدام النماذج (مخفي افتراضيا). مرّر رقمًا لعرض أعلى N         |
|  | التصفية حسب المشروع (الافتراضي: كل المشاريع، سلسلة فارغة: المشروع الحالي) |
...
| الراية                                      | المختصر | الوصف                               |
| ------------------------------------------- | ------- | ----------------------------------- |
|  | `-c`    | الإبقاء على ملفات التهيئة           |
|    | `-d`    | الإبقاء على بيانات الجلسات واللقطات |
|      |         | عرض ما سيتم حذفه دون تنفيذ الحذف    |
|        | `-f`    | تخطي مطالبات التأكيد                |
...
| الراية                                 | المختصر | الوصف                                                       |
| -------------------------------------- | ------- | ----------------------------------------------------------- |
|  | `-m`    | طريقة التثبيت المستخدمة: curl أو npm أو pnpm أو bun أو brew |
