...
| ---------------------------------------- | --------------------------------------- | ---------------------------------------------------------- |
|  | `-c`                                    | 最後のセッションを続行                                     |
|   |                                         | 続行時にセッションをフォーク (`-s` または `--fork` と併用) |
|  |  | 続行するセッション ID                                      |
|    |                                         | 使用のプロンプト                                           |
|     | `-m`                                    | プロバイダー/モデルの形式で使用するモデル                  |
|     |                                         | 使用するエージェント                                       |
|      |                                         | リッスンするポート                                         |
|  |                                         | リッスンするホスト名                                       |
...
|  | `-c`     | 最後のセッションを続行                                                            |
|   | `-s`     | 続行するセッション ID                                                             |
|      |          | 続行時にセッションをフォーク（`--continue` または `--session` と使用）            |
|  | `-p`     | Basic 認証パスワード（デフォルトは `OPENCODE_SERVER_PASSWORD`）                   |
|  | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `opencode`） |
...
| フラグ                                | 説明                                                |
| ------------------------------------- | --------------------------------------------------- |
|  | エージェントを実行するための GitHub モック イベント |
|  | GitHub 個人アクセストークン                         |
...
| フラグ                                  | 説明                                                            |
| --------------------------------------- | --------------------------------------------------------------- |
|  | models.dev からモデルキャッシュを更新します。                   |
|  | より詳細なモデル出力を使用します (コストなどのメタデータを含む) |
...
| フラグ                                   | ショート | 説明                                                                                      |
| ---------------------------------------- | -------- | ----------------------------------------------------------------------------------------- |
|   |          | 実行するコマンド。引数には message を使用します。                                         |
|  | `-c`     | 最後のセッションを続行                                                                    |
|   | `-s`     | 続行するセッション ID                                                                     |
|      |          | 続行時にセッションをフォーク (`--continue` または `--session` と併用)                     |
|     |          | セッションを共有する                                                                      |
|     | `-m`     | プロバイダー/モデルの形式で使用するモデル                                                 |
|     |          | 使用するエージェント                                                                      |
|      | `-f`     | メッセージに添付するファイル                                                              |
|    |          | 形式: デフォルト (フォーマット済み) または json (生の JSON イベント)                      |
|     |          | セッションのタイトル (値が指定されていない場合は、切り詰められたプロンプトが使用されます) |
|    |          | 実行中の opencode サーバー (http://localhost:4096 など) に接続します。                    |
|  | `-p`     | Basic 認証パスワード（デフォルトは `OPENCODE_SERVER_PASSWORD`）                           |
|  | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `opencode`）         |
|       |          | 実行ディレクトリ、またはアタッチ時のリモートサーバー上のパス                              |
|   |          | モデルバリアント（プロバイダー固有の推論レベル）                                          |
|  |          | 思考ブロックを表示                                                                        |
...
|     | 過去 N 日間の統計を表示 (全期間)                                                           |
|    | 表示するツールの数 (すべて)                                                                |
|   | モデルの使用状況の内訳を表示 (デフォルトでは非表示)。上位 N 件を表示するには数値を渡します |
|  | プロジェクトでフィルタリング (全プロジェクト、空文字列: 現在のプロジェクト)                |
...
| フラグ                                 | ショート | 説明                                                   |
| -------------------------------------- | -------- | ------------------------------------------------------ |
|  | `-m`     | 使用されたインストール方法。curl, npm, pnpm, bun, brew |
