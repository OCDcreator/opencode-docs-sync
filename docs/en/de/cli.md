...
| ---------------------------------------- | ---- | ---------------------------------------------------------------------- |
|  | `-c` | Setzen Sie die letzte Sitzung fort                                     |
|   | `-s` | Session-ID zum Fortfahren                                              |
|      |      | Sitzung beim Fortsetzen verzweigen (mit `--continue` oder `--session`) |
|    |      | Prompt zur Verwendung                                                  |
|     | `-m` | Zu verwendendes Modell in der Form provider/model                      |
...
| Flag                                     | Kurz | Beschreibung                                                                       |
| ---------------------------------------- | ---- | ---------------------------------------------------------------------------------- |
|       |      | Arbeitsverzeichnis zum Starten von TUI in                                          |
|  | `-c` | Letzte Sitzung fortsetzen                                                          |
|   | `-s` | Session-ID zum Fortfahren                                                          |
|      |      | Sitzung beim Fortsetzen abzweigen (mit `--continue` oder `--session` verwenden)    |
|  | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                     |
|  | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `opencode`) |
...
| Flag                                  | Beschreibung                                        |
| ------------------------------------- | --------------------------------------------------- |
|  | GitHub Scheinereignis zum Ausführen des Agenten für |
...
| Flag                                    | Beschreibung                                                                          |
| --------------------------------------- | ------------------------------------------------------------------------------------- |
|  | Aktualisieren Sie den Modellcache von models.dev                                      |
|  | Verwenden Sie eine ausführlichere Modellausgabe (einschließlich Metadaten wie Kosten) |
...
| Flag                                     | Kurz | Beschreibung                                                                                        |
| ---------------------------------------- | ---- | --------------------------------------------------------------------------------------------------- |
|   |      | Der auszuführende Befehl, Argumente als Nachricht verwenden                                         |
|  | `-c` | Setzen Sie die letzte Sitzung fort                                                                  |
|   | `-s` | Session-ID zum Fortfahren                                                                           |
|      |      | Sitzung beim Fortsetzen verzweigen (mit `--continue` oder `--session` verwenden)                    |
|     |      | Teilen Sie die Sitzung                                                                              |
|     | `-m` | Zu verwendendes Modell in der Form provider/model                                                   |
|     |      | Zu verwendender Agent                                                                               |
|      | `-f` | Datei(en) zum Anhängen an die Nachricht                                                             |
|    |      | Format: default (formatiert) oder json (rohe JSON-Ereignisse)                                       |
|     |      | Titel für die Sitzung (verwendet eine verkürzte Eingabeaufforderung, wenn kein Wert angegeben wird) |
|    |      | An einen laufenden OpenCode-Server anschließen (z.B. http://localhost:4096)                         |
|  | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
|  | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `opencode`)                  |
|       |      | Verzeichnis für die Ausführung, oder Pfad auf dem Remote-Server beim Anhängen                       |
|   |      | Modellvariante (anbieterspezifischer Reasoning-Aufwand)                                             |
|  |      | Denkblöcke anzeigen                                                                                 |
|      |      | Port für den lokalen Server (standardmäßig zufälliger Port)                                         |
...
|      | Port zum Abhören                                |
|  | Hostname zum Abhören                            |
|      | mDNS-Discovery aktivieren                       |
|      | Zusätzliche Browser-Ursprünge für CORS zulassen |
...
| Flag                                      | Kurz | Beschreibung                             |
| ----------------------------------------- | ---- | ---------------------------------------- |
|  | `-n` | Beschränken auf die N neuesten Sitzungen |
|     |      | Ausgabeformat: table oder json (table)   |
...
| Flag                                    | Beschreibung                                                                                                                   |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
|     | Statistiken für die letzten N Tage anzeigen (alle Zeiten)                                                                      |
|    | Anzahl der angebotenen Werkzeuge (alle)                                                                                        |
|   | Aufschlüsselung der Modellnutzung anzeigen (standardmäßig ausgeblendet). Übergeben Sie eine Zahl, um die obersten N anzuzeigen |
|  | Nach Projekt filtern (alle Projekte, leere Zeichenfolge: aktuelles Projekt)                                                    |
...
| Flag                                        | Kurz | Beschreibung                                        |
| ------------------------------------------- | ---- | --------------------------------------------------- |
|  | `-c` | Konfigurationsdateien behalten                      |
|    | `-d` | Sitzungsdaten und Snapshots aufbewahren             |
|      |      | Zeigt, was entfernt werden würde, ohne zu entfernen |
|        | `-f` | Bestätigungsaufforderungen überspringen             |
...
| Flag                                   | Kurz | Beschreibung                                                        |
| -------------------------------------- | ---- | ------------------------------------------------------------------- |
|  | `-m` | Die zu verwendende Installationsmethode; curl, npm, pnpm, bun, brew |
...
|  |      | Protokolle nach stderr drucken            |
