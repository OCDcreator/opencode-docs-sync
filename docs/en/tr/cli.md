...
| ---------------------------------------- | ---- | --------------------------------------------------------------------------- |
|  | `-c` | Son oturuma devam et                                                        |
|   | `-s` | Devam edecek oturum kimliği                                                 |
|      |      | Devam ederken oturumu fork'lar (`--continue` veya `--session` ile kullanın) |
|    |      | Kullanılacak prompt                                                         |
|     | `-m` | provider/model biçiminde kullanılacak model                                 |
|     |      | Kullanılacak agent                                                          |
|      |      | Dinlenecek port                                                             |
|  |      | Dinlenecek host adı                                                         |
...
| Bayrak                                   | Kısa | Tanım                                                                                         |
| ---------------------------------------- | ---- | --------------------------------------------------------------------------------------------- |
|       |      | TUI'yi başlatmak için çalışma dizini                                                          |
|  | `-c` | Son oturuma devam et                                                                          |
|   | `-s` | Devam edecek oturum açma bilgileri                                                            |
|      |      | Devam ederken oturumu çatalla (`--continue` veya `--session` ile kullanın)                    |
|  | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
|  | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `opencode`) |
...
| Bayrak                                | Açıklama                                    |
| ------------------------------------- | ------------------------------------------- |
|  | Aracıyı çalıştırmak için GitHub sahte olayı |
...
| Bayrak                                  | Tanım                                                                       |
| --------------------------------------- | --------------------------------------------------------------------------- |
|  | Modeller.dev'den model önbelleğini yenileyin                                |
|  | Daha ayrıntılı model çıktısı kullanın (maliyetler gibi meta veriler içerir) |
...
| Bayrak                                   | Kısa | Açıklama                                                                                      |
| ---------------------------------------- | ---- | --------------------------------------------------------------------------------------------- |
|   |      | Çalıştırılacak komut, args için mesajı kullanın                                               |
|  | `-c` | Son oturuma devam et                                                                          |
|   | `-s` | Devam edecek oturum kimliği                                                                   |
|      |      | Devam ederken oturumu fork'lar (`--continue` veya `--session` ile kullanın)                   |
|     |      | Oturumu paylaşın                                                                              |
|     | `-m` | provider/model biçiminde kullanılacak model                                                   |
|     |      | Kullanılacak temsilci                                                                         |
|      | `-f` | Mesaja eklenecek dosya(lar)                                                                   |
|    |      | Biçim: varsayılan (biçimlendirilmiş) veya json (ham JSON olayları)                            |
|     |      | Oturumun başlığı (değer sağlanmazsa kısaltılmış bilgi istemi kullanılır)                      |
|    |      | Çalışan bir opencode sunucusuna ekleyin (ör. http://localhost:4096)                           |
|  | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
|  | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `opencode`) |
|       |      | Çalıştırılacak dizin veya bağlanırken uzak sunucudaki yol                                     |
|   |      | Model varyantı (sağlayıcıya özgü muhakeme düzeyi)                                             |
|  |      | Düşünme bloklarını göster                                                                     |
|      |      | Yerel sunucunun bağlantı noktası (varsayılan olarak rastgele bağlantı noktasıdır)             |
...
| Bayrak                                   | Tanım                                      |
| ---------------------------------------- | ------------------------------------------ |
|      | Dinlenecek bağlantı noktası                |
|  | Dinlenecek ana bilgisayar adı              |
|      | mDNS bulmayı etkinleştir                   |
|      | CORS'a izin verecek ek tarayıcı kaynakları |
...
| Bayrak                                    | Kısa | Tanım                                  |
| ----------------------------------------- | ---- | -------------------------------------- |
|  | `-n` | En son N oturumla sınırla              |
|     |      | Çıkış formatı: tablo veya json (tablo) |
...
| Bayrak                                  | Açıklama                                                                                                    |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
|     | Son N güne ait istatistikleri göster (tüm zamanlar)                                                         |
|    | Gösterilecek araç sayısı (tümü)                                                                             |
|   | Model kullanım dökümünü göster (varsayılan olarak gizlidir). En üstteki N'yi göstermek için bir sayı iletin |
|  | Projeye göre filtrele (tüm projeler, boş değer: mevcut proje)                                               |
...
| Bayrak                                      | Kısa | Tanım                                           |
| ------------------------------------------- | ---- | ----------------------------------------------- |
|  | `-c` | Yapılandırma dosyalarını sakla                  |
|    | `-d` | Oturum verilerini ve anlık görüntüleri saklayın |
|      |      | Nelerin kaldırılmadan kaldırılacağı göster      |
|        | `-f` | Onay istemlerini atla                           |
...
| Bayrak                                 | Kısa | Açıklama                                               |
| -------------------------------------- | ---- | ------------------------------------------------------ |
|  | `-m` | Kullanılan kurulum yöntemi: curl, npm, pnpm, bun, brew |
...
|  |      | Günlükleri stderr'e yazdır               |
|   |      | Günlük düzeyi (DEBUG, INFO, WARN, ERROR) |
