...
| ---------------------------------------- | ----- | ------------------------------------------------------------------------ |
|  | `-c`  | Continua l'ultima sessione                                               |
|   | `-s`  | ID sessione da continuare                                                |
|      |       | Duplica la sessione quando continui (usa con `--continue` o `--session`) |
|    |       | Prompt da usare                                                          |
|     | `-m`  | Modello nel formato provider/model                                       |
|     |       | Agente da usare                                                          |
|      |       | Porta su cui mettersi in ascolto                                         |
|  |       | Hostname su cui mettersi in ascolto                                      |
...
| Flag                                     | Breve | Descrizione                                                                                     |
| ---------------------------------------- | ----- | ----------------------------------------------------------------------------------------------- |
|       |       | Working directory in cui avviare la TUI                                                         |
|  | `-c`  | Continua l'ultima sessione                                                                      |
|   | `-s`  | ID sessione da continuare                                                                       |
|      |       | Crea un fork della sessione durante la continuazione (usa con `--continue` o `--session`)       |
|  | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
|  | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `opencode`) |
...
| Flag                                  | Descrizione                                  |
| ------------------------------------- | -------------------------------------------- |
|  | Evento GitHub mock per cui eseguire l'agente |
...
| Flag                                    | Descrizione                                        |
| --------------------------------------- | -------------------------------------------------- |
|  | Aggiorna la cache modelli da models.dev            |
|  | Output più verboso (include metadati come i costi) |
...
| Flag                                     | Breve | Descrizione                                                                                     |
| ---------------------------------------- | ----- | ----------------------------------------------------------------------------------------------- |
|   |       | Il comando da eseguire; usa message per gli argomenti                                           |
|  | `-c`  | Continua l'ultima sessione                                                                      |
|   | `-s`  | ID sessione da continuare                                                                       |
|      |       | Duplica la sessione quando continui (usa con `--continue` o `--session`)                        |
...
|     | `-m`  | Modello nel formato provider/model                                                              |
|     |       | Agente da usare                                                                                 |
|      | `-f`  | File da allegare al messaggio                                                                   |
|    |       | Formato: default (formattato) o json (eventi JSON grezzi)                                       |
|     |       | Titolo sessione (usa prompt troncato se non viene fornito un valore)                            |
|    |       | Attach a un server opencode in esecuzione (es. http://localhost:4096)                           |
|  | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
|  | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `opencode`) |
|       |       | Directory di esecuzione, o percorso sul server remoto durante il collegamento                   |
|   |       | Variante del modello (sforzo di ragionamento specifico del provider)                            |
|  |       | Mostra blocchi di pensiero                                                                      |
|      |       | Porta per il server locale (di default una porta casuale)                                       |
...
|      | Porta su cui mettersi in ascolto               |
|  | Hostname su cui mettersi in ascolto            |
|      | Abilita discovery mDNS                         |
|      | Origin browser addizionali per consentire CORS |
...
| Flag                                      | Breve | Descrizione                          |
| ----------------------------------------- | ----- | ------------------------------------ |
|  | `-n`  | Limita alle N sessioni più recenti   |
|     |       | Formato output: table o json (table) |
...
| Flag                                    | Descrizione                                                                           |
| --------------------------------------- | ------------------------------------------------------------------------------------- |
|     | Mostra statistiche per gli ultimi N giorni (all time)                                 |
|    | Numero di strumenti da mostrare (all)                                                 |
|   | Mostra breakdown di utilizzo modelli (nascosto di default). Passa un numero per top N |
|  | Filtra per progetto (tutti i progetti; stringa vuota: progetto corrente)              |
...
| Flag                                        | Breve | Descrizione                                  |
| ------------------------------------------- | ----- | -------------------------------------------- |
|  | `-c`  | Mantieni i file di configurazione            |
|    | `-d`  | Mantieni dati di sessione e snapshot         |
|      |       | Mostra cosa verrebbe rimosso senza rimuovere |
|        | `-f`  | Salta le richieste di conferma               |
...
| Flag                                   | Breve | Descrizione                                               |
| -------------------------------------- | ----- | --------------------------------------------------------- |
|  | `-m`  | Metodo di installazione usato: curl, npm, pnpm, bun, brew |
