...
|  | `-c` | 마지막 세션 이어서 실행                                                |
|   | `-s` | 이어서 실행할 세션 ID                                                  |
|      |      | 세션을 이어갈 때 포크 생성 (`--continue` 또는 `--session`과 함께 사용) |
|    |      | 사용할 프롬프트                                                        |
|     | `-m` | 사용할 모델 (`provider/model` 형식)                                    |
|     |      | 사용할 에이전트                                                        |
|      |      | 수신 포트                                                              |
|  |      | 수신 호스트명                                                          |
...
|  | `-c` | 마지막 세션 이어서 실행                                                    |
|   | `-s` | 이어서 실행할 세션 ID                                                      |
|      |      | 이어서 실행할 때 세션 포크 (`--continue` 또는 `--session`과 함께 사용)     |
|  | `-p` | 기본 인증 비밀번호 (기본값: `OPENCODE_SERVER_PASSWORD`)                    |
|  | `-u` | 기본 인증 사용자 이름 (기본값: `OPENCODE_SERVER_USERNAME` 또는 `opencode`) |
...
| 플래그                                  | 설명                                              |
| --------------------------------------- | ------------------------------------------------- |
|  | models.dev에서 모델 캐시 새로고침                 |
|  | 더 자세한 모델 출력 사용(비용 등 메타데이터 포함) |
...
|     |      | 사용할 에이전트                                                            |
|      | `-f` | 메시지에 첨부할 파일                                                       |
|    |      | 출력 형식: default(포맷됨) 또는 json(원시 JSON 이벤트)                     |
|     |      | 세션 제목(값이 없으면 프롬프트를 잘라 자동 생성)                           |
|    |      | 실행 중인 opencode 서버에 연결(예: http://localhost:4096)                  |
|  | `-p` | 기본 인증 비밀번호 (기본값: `OPENCODE_SERVER_PASSWORD`)                    |
|  | `-u` | 기본 인증 사용자 이름 (기본값: `OPENCODE_SERVER_USERNAME` 또는 `opencode`) |
|       |      | 실행할 디렉터리, 또는 연결 시 원격 서버 경로                               |
|   |      | 모델 변형 (제공자별 추론 수준)                                             |
|  |      | 사고 블록 표시                                                             |
|      |      | 로컬 서버 포트(기본값: 랜덤 포트)                                          |
...
| 플래그                                  | 설명                                                         |
| --------------------------------------- | ------------------------------------------------------------ |
|     | 최근 N일 통계 표시(기본값: 전체 기간)                        |
|    | 표시할 도구 개수(기본값: 전체)                               |
|   | 모델 사용량 상세 표시(기본 숨김). 숫자를 주면 상위 N개 표시  |
|  | 프로젝트 필터(기본: 전체 프로젝트, 빈 문자열: 현재 프로젝트) |
...
| 플래그                                 | 축약 | 설명                                       |
| -------------------------------------- | ---- | ------------------------------------------ |
|  | `-m` | 설치 방식 지정: curl, npm, pnpm, bun, brew |
