...
| ---------------------------------------- | ---- | ---------------------------------------------------------------------------- |
|  | `-c` | Fortsæt sidste session                                                       |
|   | `-s` | Sessions-id for at fortsætte                                                 |
|      |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`) |
|    |      | Spørg om at bruge                                                            |
|     | `-m` | Model til brug i form af provider/model                                      |
|     |      | Agent hos bruge                                                              |
|      |      | Port at lytte på                                                             |
...
|  | `-c` | Fortsæt den seneste session                                                       |
|   | `-s` | Sessions-id for at fortsætte                                                      |
|      |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`)      |
|  | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                 |
|  | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `opencode`) |
...
|     | `-m` | Model til brug i form af provider/model                                             |
|     |      | Agent til brug                                                                      |
|      | `-f` | Fil(er), der skal vedhæftes til meddelelsen                                         |
...
|    |      | Tilslut til en kørende opencode-server (f.eks. http://localhost:4096)               |
|  | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
|  | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `opencode`)   |
|       |      | Mappe at køre i, eller sti på fjernserveren ved tilkobling                          |
|   |      | Modelvariant (udbyder-specifik ræsonneringsindsats)                                 |
|  |      | Vis tænkeblokke                                                                     |
...
| Flag                                      | Kort | Beskrivelse                            |
| ----------------------------------------- | ---- | -------------------------------------- |
|  | `-n` | Begræns til N seneste sessioner        |
...
| Flag                                        | Kort | Beskrivelse                                      |
| ------------------------------------------- | ---- | ------------------------------------------------ |
|  | `-c` | Se konfigurationsfiler                           |
|    | `-d` | Gem sessionsdata og snapshots                    |
|      |      | Vis, hvad der ville blive fjernet uden at fjerne |
|        | `-f` | Spring bekræftelsesspørgsmål over                |
...
| Flag                                   | Kort | Beskrivelse                                                      |
| -------------------------------------- | ---- | ---------------------------------------------------------------- |
|  | `-m` | Installationsmetoden, der blev brugt; curl, npm, pnpm, bun, brew |
