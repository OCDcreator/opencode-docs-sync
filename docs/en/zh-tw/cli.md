...
|  | `-c` | 繼續上一個工作階段                                            |
|   | `-s` | 要繼續的工作階段 ID                                           |
|      |      | 繼續時分岔工作階段（與 `--continue` 或 `--session` 搭配使用） |
|    |      | 要使用的提示詞                                                |
|     | `-m` | 要使用的模型，格式為 provider/model                           |
|     |      | 要使用的代理                                                  |
|      |      | 監聽連接埠                                                    |
|  |      | 監聽主機名稱                                                  |
...
|  | `-c` | 繼續上一個工作階段                                                      |
|   | `-s` | 要繼續的工作階段 ID                                                     |
|      |      | 繼續時分支工作階段（與 `--continue` 或 `--session` 一起使用）           |
|  | `-p` | 基本驗證密碼（預設使用 `OPENCODE_SERVER_PASSWORD`）                     |
|  | `-u` | 基本驗證使用者名稱（預設使用 `OPENCODE_SERVER_USERNAME` 或 `opencode`） |
...
| 旗標                                    | 說明                                       |
| --------------------------------------- | ------------------------------------------ |
|  | 從 models.dev 重新整理模型快取             |
|  | 使用更詳細的模型輸出（包含費用等中繼資料） |
...
|     |      | 要使用的代理                                                            |
|      | `-f` | 附加到訊息的檔案                                                        |
|    |      | 格式：default（格式化輸出）或 json（原始 JSON 事件）                    |
|     |      | 工作階段標題（未提供值時使用截斷的提示詞）                              |
|    |      | 連接到正在執行的 opencode 伺服器（例如 http://localhost:4096）          |
|  | `-p` | 基本驗證密碼（預設使用 `OPENCODE_SERVER_PASSWORD`）                     |
|  | `-u` | 基本驗證使用者名稱（預設使用 `OPENCODE_SERVER_USERNAME` 或 `opencode`） |
|       |      | 執行目錄，或附加時遠端伺服器上的路徑                                    |
|   |      | 模型變體（特定於提供者的推理級別）                                      |
...
| 旗標                                    | 說明                                                 |
| --------------------------------------- | ---------------------------------------------------- |
|     | 顯示最近 N 天的統計資訊（預設為所有時間）            |
|    | 顯示的工具數量（預設為全部）                         |
|   | 顯示模型用量明細（預設隱藏）。傳入數字可顯示前 N 個  |
|  | 按專案篩選（預設為所有專案，傳入空字串表示當前專案） |
...
| 旗標                                   | 簡寫 | 說明                                       |
| -------------------------------------- | ---- | ------------------------------------------ |
|  | `-m` | 使用的安裝方式：curl、npm、pnpm、bun、brew |
