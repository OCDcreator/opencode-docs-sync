...
|  | `-p` | 基本认证密码（默认使用 `OPENCODE_SERVER_PASSWORD`）                 |
|  | `-u` | 基本认证用户名（默认使用 `OPENCODE_SERVER_USERNAME` 或 `opencode`） |
...
|     |      | 会话标题（未提供值时使用截断的提示词）                              |
|    |      | 连接到正在运行的 opencode 服务器（例如 http://localhost:4096）      |
|  | `-p` | 基本认证密码（默认使用 `OPENCODE_SERVER_PASSWORD`）                 |
|  | `-u` | 基本认证用户名（默认使用 `OPENCODE_SERVER_USERNAME` 或 `opencode`） |
|       |      | 运行目录，或附加时远程服务器上的路径                                |
|   |      | 模型变体（特定于提供商的推理级别）                                  |
...
| 标志                                   | 简写 | 描述                                       |
| -------------------------------------- | ---- | ------------------------------------------ |
|  | `-m` | 使用的安装方式：curl、npm、pnpm、bun、brew |
